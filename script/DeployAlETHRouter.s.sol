...
import {AlETHRouter, IAlchemistV2, ICurvePool, ICurveCalc} from "../src/AlETHRouter.sol";
...
contract DeployAlETHRouter is Script {
    ...
    function run() external returns (AlETHRouter) {
        ...
        return deploy(config.alchemist, config.alETHPool, config.curveCalc);
    }

    /// @dev Deploy the contract.
    function deploy(IAlchemistV2 alchemist, ICurvePool alETHPool, ICurveCalc curveCalc) public returns (AlETHRouter) {
